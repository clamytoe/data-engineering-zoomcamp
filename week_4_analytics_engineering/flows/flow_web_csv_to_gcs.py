...
class DataFrameSerializationFormat(Enum):
    ...
    def fix_extension_with(self, gcs_blob_path: str) -> str:
        ...
        """
        gcs_blob_path = Path(gcs_blob_path)  # type: ignore
        folder = gcs_blob_path.parent  # type: ignore
        filename = Path(gcs_blob_path.stem).with_suffix(self.suffix)  # type: ignore
...
def upload_from_dataframe(
    ...
    **upload_kwargs: Dict[str, Any],
    ...
    with BytesIO() as bytes_buffer:
        ...
        return gcs_bucket.upload_from_file_object(  # type: ignore
...
def ingest_csv_to_gcs():
    ...
    for dataset_name, endpoints in datasets.items():
        ...
        for endpoint in endpoints:
            ...
            cleansed_df = fix_datatypes_for(df=raw_df, schema=schemas.get(dataset_name))  # type: ignore
            upload_from_dataframe(
                prefect_gcs_block=prefect.gcs_block_name,
                df=cleansed_df,
                to_path=gcs_path,
            )
